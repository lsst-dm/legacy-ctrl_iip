...
void show_manual() { 
    ...
    cout << "Usage: ./emulator log <device> <log_event>" << endl; 
    ...
    cout << "log: " << endl; 
    cout << "     ./emulator log AT summaryState" << endl; 
    cout << "     ./emulator log AT recommendedSettingsVersions" << endl; 
    cout << "     ./emulator log AT settingsApplied" << endl; 
    cout << "     ./emulator log AT appliedSettingsMatchStart" << endl; 
    cout << "     ./emulator log AT errorCode" << endl; 
} 
...
void send_command_at(string command, Node n) { 
    ...
    if (command == "all") { 
        ...
        atStart.settingsToApply = "Normal";
...
int main(int argc, char *argv[]) { 
    if (argc < 3) { 
        ...
        if (device == "AT") { } 
            ...
            string log_device(argv[2]); 
            string log_event(argv[3]); 
            SAL<SAL_archiver, SAL_atArchiver> mgr;
            if (log_event  == "summaryState") { 
                mgr.log_summary_state(); 
            } 
            else if (log_event  == "recommendedSettingsVersions") { 
                mgr.log_recommended_settings_versions(); 
            } 
            else if (log_event  == "settingsApplied") { 
                mgr.log_settings_applied(); 
            } 
            else if (log_event  == "appliedSettingsMatchStart") { 
                mgr.log_applied_settings_match_start();
            } 
            else if (log_event  == "errorCode") { 
                mgr.log_error_code(); 
            } 
        } 
        else { 
