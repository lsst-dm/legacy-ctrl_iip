...
/*
 * This file contains the implementation for the catchuparchiver_standby commander test.
 *
 ***/
...
{ 
  ...
  catchuparchiver_command_standbyC myData;
  if (argc < 2 ) {
  ...
  ostringstream url; 
  url << "amqp://CL_13:CL_13@" << broker; 
  cout << url.str() << endl; 
  SimplePublisher *publisher = new SimplePublisher(url.str()); 


  mgr.salCommand("catchuparchiver_command_standby");
  ...
  if (status == 303) { // completed_ok
     publisher->publish_message("test_dmcs_ocs_publish", "{ MSG_TYPE: STANDBY_ACK, DEVICE: CU, CMD_ID: None, ACK_ID: None, ACK_BOOL: None, ACK_STATEMENT }");  
  } 
