...
{ 
  ...
  string user_name = user_file["atArchiver"]["standby"].as<string>(); 
  ostringstream url; 
  url << "amqp://" 
      << user_name 
      << ":"
  ...
  if (status == 303) { // completed_ok
     publisher->publish_message("test_dmcs_ocs_publish", "{ MSG_TYPE: STANDBY_ACK, DEVICE: AT, CMD_ID: None, ACK_ID: None, ACK_BOOL: None, ACK_STATEMENT: None }");  
  } 
