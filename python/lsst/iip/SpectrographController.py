...
class SpectrographController:

    SPECTRO_CTRL_PUBLISH = "spectro_ctrl_publish"
    SPECTRO_CTRL_CONSUME = "spectro_ctrl_consume"
    ACK_PUBLISH = "sp_foreman_ack_publish"
    AUDIT_CONSUME = "audit_consume"
    YAML = 'YAML'
    RECEIPT_FILE = "/var/spectro/spectro_controller_receipt"



    def __init__(self, filename=None):
        self._session_id = None
        self._name = "SPECTRO_CTRL"
        ...
        try:
            self._spectro_name = cdm[ROOT]['SPECTRO_BROKER_NAME']  # Message broker user/passwd for component
            self._spectro_passwd = cdm[ROOT]['SPECTRO_BROKER_PASSWD']
            self._base_broker_addr = cdm[ROOT][BASE_BROKER_ADDR]
            self._spectro_xfer_root = cdm[ROOT]['SPECTRO']['SPECTRO_XFER_ROOT']
            if cdm[ROOT]['SPECTRO']['CHECKSUM_ENABLED'] == 'yes':
        ...
        self._base_broker_url = "amqp://" + self._spectro_name + ":" + self._spectro_passwd + "@" + str(self._base_broker_addr)

        LOGGER.info('Building _base_broker_url connection string for Spectro Controller. Result is %s', 
                     self._base_broker_url)

        self._msg_actions = { 'SPECTRO_HEALTH_CHECK': self.process_health_check,
                              'NEW_SPECTRO_ITEM': self.process_new_spectro_item,
                              'SP_ITEMS_XFERD': self.process_transfer_complete }
    ...
    def setup_consumer(self):
        LOGGER.info('Setting up spectro consumers on %s', self._base_broker_url)
        LOGGER.info('Running start_new_thread for spectro consumer')

        self._spectro_consumer = Consumer(self._base_broker_url, self.SPECTRO_CTRL_CONSUME, self._base_msg_format)
        try:
            _thread.start_new_thread( self.run_spectro_consumer, ("thread-spectro-consumer", 2,) )
        except:
            LOGGER.critical('Cannot start Archive consumer thread, exiting...')
            raise L1Error

    def run_spectro_consumer(self, threadname, delay):
        self._spectro_consumer.run(self.on_spectro_message)



    def setup_publisher(self):
        LOGGER.info('Setting up Archive publisher on %s using %s', self._base_broker_url, self._base_msg_format)
        self._spectro_publisher = SimplePublisher(self._base_broker_url, self._base_msg_format)
        #self._audit_publisher = SimplePublisher(self._base_broker_url, self._base_msg_format)



    def on_spectro_message(self, ch, method, properties, msg_dict):
    ...
    def process_health_check(self, params):
        ...
        self.send_health_ack_response("SPECTRO_HEALTH_CHECK_ACK", params)
        

    def process_new_spectro_item(self, params):
    ...
    def send_health_ack_response(self, type, params):
        ...
        self._spectro_publisher.publish_message(self.ACK_PUBLISH, msg_params)
    ...
    def send_audit_message(self, prefix, params):
        ...
        self._spectro_publisher.publish_message(self.AUDIT_CONSUME, audit_params)
    ...
    def construct_send_target_dir(self, params):
        ...
        target_dir_visit = self._spectro_xfer_root + visit + "/"
        target_dir_image = self._spectro_xfer_root + visit + "/" + str(image) + "/"
    ...
    def send_new_item_ack(self, target_dir, params):
        ack_params = {}
        ack_params[MSG_TYPE] = 'NEW_SPECTRO_ITEM_ACK'
        ...
        self._spectro_publisher.publish_message(self.ACK_PUBLISH, ack_params)
    ...
    def send_transfer_complete_ack(self, transfer_results, params):
        ...
        ack_params['MSG_TYPE'] = 'SP_ITEMS_XFERD_ACK'
        ack_params['COMPONENT'] = self._name
        ack_params['ACK_ID'] = params['ACK_ID']
        ack_params['ACK_BOOL'] = True
