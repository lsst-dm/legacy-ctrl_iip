...
Forwarder::Forwarder() {
    ...
    try {
        LOGGER(my_logger::get(), debug) << "Reading config values from ForwarderCfg file."; 
...
void Forwarder::setup_consumers(string BASE_BROKER_ADDR){
    ...
    LOGGER(my_logger::get(), debug) << full_broker_url.str();
    ...
    LOGGER(my_logger::get(), debug) << from_fwd_broker_url.str();
...
void Forwarder::run() {
    ...
    LOGGER(my_logger::get(), info) << "Finished setting up threads for consumers."; 
}
...
void *Forwarder::run_thread(void *pargs) {
    ...
    LOGGER(my_logger::get(), info) << "Finished setting up thread."; 
}
...
void Forwarder::setup_publishers(string BASE_BROKER_ADDR){
    ...
    LOGGER(my_logger::get(), debug) << "Pub Broker url is: " << full_broker_url.str();
    ...
    LOGGER(my_logger::get(), debug) << "Pub Broker url is: " << full_broker_url1.str();
    ...
    LOGGER(my_logger::get(), debug) << "Pub Broker url is: " << full_broker_url2.str();
    ...
    LOGGER(my_logger::get(), debug) << "Pub Broker url is: " << full_broker_url3.str();
    ...
    LOGGER(my_logger::get(), debug) << "Pub Broker url is: " << full_broker_url4.str();
    ...
    LOGGER(my_logger::get(), debug) << "Pub Broker url is: " << full_broker_url5.str();
    ...
    LOGGER(my_logger::get(), debug) << "Pub Broker url is: " << full_broker_url6.str();
    fwd_pub = new SimplePublisher(full_broker_url6.str());
    LOGGER(my_logger::get(), info) << "Setting up publishers is complete."; 
}
...
void Forwarder::on_forwarder_to_fetch_message(string body) {
    LOGGER(my_logger::get(), debug) << "In fETCHr callback that receives msgs from main forwarder thread";
    LOGGER(my_logger::get(), debug) << "-----------Message Body Is:------------";
    LOGGER(my_logger::get(), debug) << body;
    LOGGER(my_logger::get(), debug) << "----------------------";
    Node node = Load(body);
...
void Forwarder::process_xfer_params(Node n) {
    ...
    LOGGER(my_logger::get(), debug) << "Incoming Node n is " << n;

    Node p = n["XFER_PARAMS"];
    LOGGER(my_logger::get(), debug) << "Sub Node p is " << p;

    this->visit_raft_list.clear();
    this->visit_raft_list = p["RAFT_LIST"].as<vector<string>>();
    LOGGER(my_logger::get(), debug) << "In process_xfer_params, RAFT_LIST has been ASSIGNED to class var.";

    this->visit_ccd_list_by_raft.clear();
    this->visit_ccd_list_by_raft = p["RAFT_CCD_LIST"].as<std::vector<std::vector<string>>>();
    LOGGER(my_logger::get(), debug) << "In process_xfer_params, RAFT_CCC_LIST has been ASSIGNED to class var";

    this->Session_ID = n["SESSION_ID"].as<string>();
    LOGGER(my_logger::get(), debug) << "After setting SESSION_ID";

    this->Job_Num = n["JOB_NUM"].as<string>();
    LOGGER(my_logger::get(), debug) << "After setting JOB_NUM";

    this->Target_Location = n["TARGET_LOCATION"].as<string>();
    LOGGER(my_logger::get(), debug) << "After setting TARGET_LOCATION";

    string reply_queue = n["REPLY_QUEUE"].as<string>();
    LOGGER(my_logger::get(), debug) << "After extracting REPLY_QUEUE";

    string ack_id = n["ACK_ID"].as<string>();
    LOGGER(my_logger::get(), debug) << "After extracting ACK_ID";
    ...
    LOGGER(my_logger::get(), debug) << "AR_XFER_PARAMS_ACK Message content is " << message.str(); 
    return;
}
...
void Forwarder::process_at_xfer_params(Node n) {
    ...
    this->Target_Location = n["TARGET_LOCATION"].as<string>();
    LOGGER(my_logger::get(), debug) << "After setting TARGET_LOCATION " << this->Target_Location;

    string reply_queue = n["REPLY_QUEUE"].as<string>();
    LOGGER(my_logger::get(), debug) << "After extracting REPLY_QUEUE " << reply_queue;

    string ack_id = n["ACK_ID"].as<string>();
    LOGGER(my_logger::get(), debug) << "After extracting ACK_ID " << ack_id;
    ...
    FWDR_pub->publish_message(reply_queue, message.str());
    LOGGER(my_logger::get(), info) << "Processing transfer params is complete."; 
    LOGGER(my_logger::get(), debug) << "AT_XFER_PARAMS_ACK Message content is " << message.str(); 
    return;
}
...
void Forwarder::process_take_images_done(Node n) {
    ...
    LOGGER(my_logger::get(), debug) << "Take images done message is published to " << this->fetch_consume_queue << " and content is " << msg.str(); 
    LOGGER(my_logger::get(), info) << "Take images done message processing is complete."; 
}
...
LOGGER(my_logger::get(), debug) << "In fetch_reassemble_raft_image, doing raft:  " << raft;
#endif
...
void Forwarder::fetch_set_up_filehandles( std::vector<std::ofstream*> &fh_set, string image_id, string raft, string ccd, string dir_prefix){
  ...
  for (int i=0; i < 16; i++) {
        ...
        LOGGER(my_logger::get(), debug) << "Filenames for handlers are " << fns.str(); 

        std::ofstream * fh = new std::ofstream(fns.str(), std::ios::out | std::ios::app | std::ios::binary );
        fh_set.push_back(fh); 
  }
    LOGGER(my_logger::get(), info) << "Finished setting up file handlers for image segments."; 
}
...
void Forwarder::fetch_close_filehandles(std::vector<std::ofstream*> &fh_set) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    LOGGER(my_logger::get(), info) << "Finished closing file handlers for image segments."; 
}
...
void Forwarder::process_header_ready(Node n) { 
    ...
    try { 
        ...
        LOGGER(my_logger::get(), info) << "Creating header directory."; 
	...
	*/ 
        ...
        LOGGER(my_logger::get(), info) << "Copied header file from efd."; 
...
void Forwarder::format_process_end_readout(Node node) { 
    ...
    try { 
        ...
        LOGGER(my_logger::get(), info) << "Looking work for current Readout pixels is complete."; 
    } 
    catch (exception& e) { 
        LOGGER(my_logger::get(), critical) << e.what() << endl; 
    } 
...
void Forwarder::format_write_img(string img_id, string raft_name, string ccd_name, string header) { 
    ...
    try { 
        ...
        if (FILE *file = fopen(destination.c_str(), "r")) { 
            ...
            LOGGER(my_logger::get(), info) << "Already existed fits file is deleted from directory."; 
        } 
        ...
        for (it = file_names.begin(); it != file_names.end(); it++) { 
            ...
            LOGGER(my_logger::get(), debug) << "Getting image pixels from each segment."; 
            ...
            LOGGER(my_logger::get(), debug) << "Copying header record rows into final fits file.";  
            ...
            LOGGER(my_logger::get(), debug) << "Cleaning up opened files."; 
...
void Forwarder::format_send_completed_msg(string image_id) { 
    ...
    try { 
        ...
        LOGGER(my_logger::get(), info) << "Formatting completed. Sending message to consume queue."; 
        ...
        if (this->readout_img_ids.size() != 0 && this->header_info_dict.size() != 0) { 
            for (it = this->readout_img_ids.begin(); it != this->readout_img_ids.end(); ) { 
                ...
                if (mit != this->header_info_dict.end()) { 
                    ...
                    LOGGER(my_logger::get(), info) << "Got both image and header data. Processing image.";  
                } 
                else it++; 
            } 
        } 
...
void Forwarder::forward_process_end_readout(Node n) { 
	...
	LOGGER(my_logger::get(), info) << "File is copied from " << img_path << " to " << dest_path; 
...
void Forwarder::forward_process_take_images_done(Node n) { 
    ...
    try { 
        ...
        LOGGER(my_logger::get(), info)  << "Msg is replied to queue ..." << reply_queue;
    } 
    catch (exception &e) { 
        LOGGER(my_logger::get(), critical) << e.what(); 
    } 
